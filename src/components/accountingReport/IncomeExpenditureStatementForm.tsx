...
function IncomeExpenditureStatementForm() {
  ...
  const [otherReportExcel, setOtherReportExcel] = useState(false);
  ...
  const downloadReports = (type:string) => {
    //e.preventDefault();
    if (validate()) {
      var model = setFormValue(values);
      if(type === "pdf"){
      accountingReportService.incomeExpenditureStatement(model).then((response) => {
        let result = response.data;
        let fileName = "IncomeExpenditureStatement.pdf";
        fileDownload(result, fileName);
      });
    }
    else{
      accountingReportService.incomeExpenditureStatementExcel(model).then((response) => {
        let result = response.data;
        let fileName = "IncomeExpenditureStatement.xlsx";
        fileDownload(result,fileName);
      });
    }}
  }
  ...
  return (
    <>
      <Typography variant="h5" align="center">
        Income & Expenditure A/C Statement
      </Typography>
      <form
        autoComplete="off"
        noValidate
        ...
        <Card>
          ...
          <CardActions sx={{ display: "flex", justifyContent: "center" }}>
            <Stack spacing={2} direction="row">
              <Button  type="button"  variant="contained" startIcon={<DownloadIcon />} onClick={() => downloadReports("pdf")}>
                Pdf
              </Button>
              {otherReportExcel && (
              <Button type="button" color='success' startIcon={<DownloadIcon />} variant="contained" onClick={() => downloadReports("excel")}>
                Excel
              </Button>
              )}
              <Button
...
export default IncomeExpenditureStatementForm
